function callback(a) {
	...
	if (c && c.length > 0) {
		...
		d.id = c;
		var e = d.number + " " + d.name,//  + " " + d.contacter,//add by xufangli 20210720 for add d.contacter
		...
		//add by xufangli 20210724 end//mody by michen 20170813 for add && for 修正选择后不关闭的bug
	...
	THISPAGE = {
		...
		addEvent: function() {
			...
			$(".grid-wrap").on("click", ".ui-icon-search", function(a) {
				...
				var b = "输入编号 / 名称 / 电话查询" === a.$_matchCon.val() ? "" : a.$_matchCon.val(),
