...
function(a) {
	function q(){
		a.ajax({
			...
			success: function(data){
			  if (data.status == 200) {
				var d = data.data;
				var dianpuCombo = $("#dianpulist").combo({
				  data: d,
				  value: "id",
				  text: "name",
				  width: 332,
	...
	function d() {
		// var dianpumc = a('div[class=list-item]:selected').text();
		// var dianpuid = a('div[class=list-item]:selected').attr("data-value");
		// if(dianpumc=="") 
		// {
		// 	dianpumc = a('div[class=list-item]').eq(0).text(); //店铺名称
		// 	dianpuid = a('div[class=list-item]').eq(0).attr("data-value"); //店铺名称
		// }
		var b = {
