	...
	delete b.firstLink, Public.ajaxPost("../basedata/contact/" + ("add" == oper ? "add" : "update")+"?type=10", b, function(d) {
		if (200 == d.status) {
			if (parent.parent.Public.tips({
				...
				var e = d.data.id;
				var categoryName = d.data.cCategoryName;
				d = b, d.id = e, d.customerType = categoryName, d.contacter = c.linkName || "", d.mobile = c.linkMobile || "", d.telephone = c.linkPhone || "", d.linkIm = c.linkIm || "", callback(d, oper, window)
			}
...
function getData() {
	var a = getEntriesData(),
		...
		c = {
			...
			difMoney: Public.currencyToNum($("#receiveFunds").val()) - Public.currencyToNum($("#periodReceiveFunds").val()),
