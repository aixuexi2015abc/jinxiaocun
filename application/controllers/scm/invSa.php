...
class InvSa extends CI_Controller {
	...
	public function saleList(){
		...
		foreach ($list as $arr=>$row) {
			...
			//add by xufangli 20210724 begin
			$v[$arr]['udf01']        = $row['udf01'];
			$v[$arr]['udf02']        = $row['udf02'];
			$v[$arr]['udf03']        = str_replace($row['udf03'],"NaN","");
			//add by xufangli 20210724 end
		}
		$json['status'] = 200;
		$json['msg']    = 'success'; 
		$json['data']['page']      = $page;
	...
	public function update() {
		...
		die('<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/><div style="text-align:center;"><div style="margin:50px 0px 20px 0px;"><img src="/statics/images/nopower.png" style="width:200px"/></div><div style="color:#808080;">单据不存在、或者已删除。</div></div>');  
    }
	...
	public function revsCheckInvSa() {
		...
		if (strlen($data)>0) {
			...
			if ($sql) {
				...
				$this->common_model->logs('购货单据编号：'.$data['billNo'].'的单据已被取消审核！');
				str_alert(200,'success',array('id'=>$data['id'])); 
			}
			str_alert(-1,'SQL错误'); 
		}
    ...
    public function rsBatchCheckInvSa() {
		...
		if (count($data)>0) {
			...
			if ($sql) {
				...
				$this->common_model->logs('销货单：'.$billno.'的单据已被取消审核！');
				str_alert(200,'销货单编号：'.$billno.'的单据已被取消审核！'); 
			} 
			str_alert(-1,'取消审核失败');  
		}
		str_alert(-1,'所选的销货单都是未审核，请选择已审核的销货单进行取消审核！'); 
	}
	...
	private function validform($data) {
		...
		$data['arrears'] < -100000000 && str_alert(-1,'本次欠款要为数字，请输入有效数字！'); 
	    ...
	    if ($data['id']>0) {
			...
			count($invoice)<1 && die('<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/><div style="text-align:center;"><div style="margin:50px 0px 20px 0px;"><img src="/statics/images/nopower.png" style="width:200px"/></div><div style="color:#808080;">单据不存在、或者已删除。</div></div>');
		...
		foreach ($data['entries'] as $arr=>$row) {
			...
			(float)$row['qty'] < -100000000 && str_alert(-1,'商品数量要为数字，请输入有效数字！'); 
			...
			if ($system['requiredCheckStore']==1 && $data['id']<1) {  
				 if (intval($data['transType'])==150601) {                        //销售才验证 
					if (isset($inventory[$row['invId']][$row['locationId']])) {
					    ...
					    //add by xufangli 20210719 for 组合品库存检查
