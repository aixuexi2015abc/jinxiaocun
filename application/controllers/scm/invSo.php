...
class InvSo extends CI_Controller {
    ...
    public function rsbatchcheckinvSo() {
		...
		if (count($data)>0) {
		    $this->mysql_model->get_count('invoice','(srcOrderId in('.$id.')) and isDelete=0')>0 && str_alert(-1,'有关联的销货单，不能对它进行取消审核！'); 
			$sql = $this->mysql_model->update('order',array('checked'=>0,'checkName'=>''),'(id in('.$id.'))'); 
			if ($sql) {
				$billno = array_column($data,'billNo','id');
				$billno = join(',',$billno);
				$this->common_model->logs('销货订单：'.$billno.'的单据已被取消审核！');
				str_alert(200,'订单编号：'.$billno.'的单据已被取消审核！'); 
			} 
			str_alert(-1,'取消审核失败');  
		}
		str_alert(-1,'所选的订单都是未审核，请选择已审核的订单进行取消审核！'); 
	}
	...
	public function checkInvSo() {
		...
		if (strlen($data)>0) {
			$data = $this->validform((array)json_decode($data, true));
			$this->mysql_model->get_count('invoice',array('srcOrderId'=>$data['id'],'isDelete'=>0))>0 && str_alert(-1,'有关联的销货单，不能对它进行取消审核！'); 
			$data['checked']         = 1;
	...
	public function revsCheckInvSo() {
		...
		if (strlen($data)>0) {
			...
			if ($sql) {
				$this->common_model->logs('销货单号：'.$data['billNo'].'的单据已被取消审核！');
				str_alert(200,'success',array('id'=>$data['id'])); 
			}
		}
		str_alert(-1,'提交失败'); 
