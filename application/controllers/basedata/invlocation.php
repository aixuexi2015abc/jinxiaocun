	...
	public function index(){
		...
		$jxcsys = $this->session->userdata('jxcsys');
		 if (!empty($datass['cangku']) && $jxcsys['uid']!="1"){
            $where .= ' and id in('.$datass['cangku'].') ';
        }
        //hnw end
		//$list = $this->mysql_model->get_results('storage',$where.$this->common_model->get_location_purview(1).' order by id desc');
		$list = $this->mysql_model->get_results('storage',$where.' order by id desc');
