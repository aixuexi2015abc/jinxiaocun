...
class Inventory extends CI_Controller {
	...
	public function index() {
		...
		//hnw start
        $dpid = $this->session->userdata('selectdpid');
		if($dpid==""||$dpid=="0"||$dpid==null)$dpid="0";
        $datass = $this->mysql_model->get_rows('dianpu','(id='.$dpid.')');
		$jxcsys = $this->session->userdata('jxcsys');
        if (!empty($datass['cangku']) && $jxcsys['uid']!="1"){
            $where .= ' and locationId in('.$datass['cangku'].')';
        }
        //hnw end
		if ($categoryid > 0) {
	...
	public function add(){
		...
		if ($data) {
			...
			if($info['locationId'] == "") $info['locationId'] = "0";	
	...
	public function update(){
		...
		if ($data) {
			...
			$info = array(
			    'barCode','baseUnitId','unitName','categoryId','categoryName','propertys',
				'discountRate1','discountRate2','highQty','pinYin','lowQty','name','number','purPrice','warehouseWarning',
				'remark','salePrice','spec','vipPrice','wholesalePrice','warehousePropertys','sonGoods','dopey','locationId','locationName'
			);
		...
		if (count($data)>0) {
			...
			if (strlen($data['sonGoods'])>0) {
				...
				foreach ($list as $arr=>$row) {
					...
					$v[$arr]['gid']             = intval($row['gid']);
					//add by xufangli 20210719
				}
			}
