...
class Contact extends CI_Controller {
	...
	public function index() {
		...
		//if($type == 10){
    		foreach ($list as $arr=>$row) {
    			...
    			$v[$arr]['links']        = '';
    			$v[$arr]['linkMen']      = $row['linkMans'];//add by xufangli 20210724
		...
		/*}else{
		    //add by xufangli 20210720 begin
		    $gid = 1;
		    foreach ($list as $arr=>$row) {
		        $v[$arr]['id']           = intval($row['id']);
		        $v[$arr]['number']       = $row['number'];
	...
	public function add(){
		...
		$data = elements(array(
					...
					,'periodMoney','remark','type','difMoney','taxRate'),$data,NULL);
		$sql = $this->mysql_model->insert('contact',$data);
		if ($sql) {
			$data['id'] = $sql;
			$data['cCategory'] = intval($data['cCategory']);
			$data['customerType'] = intval($data['cCategoryName']);
