...
class Home extends CI_Controller {
	...
	public function index(){
	    //add by xufangli 20210820 for 修改登录异常 begin
	    if(!strstr($_SERVER['REQUEST_URI'], 'home/index'))
	        redirect('home/index', 'refresh');
	    //add by xufangli 20210820 for 修改登录异常 end
	    $data['uid']      = $this->jxcsys['uid'];
		$data['name']     = $this->jxcsys['name'];
		$data['dianpuid']     = $this->jxcsys['dianpuid'];
		$data['dianpulist']     = $this->jxcsys['dianpulist'];  //hnw start
		$data['roleid']   = 0;
        ...
        //hnw start
        $dpid = str_enhtml($this->input->get('dpid',TRUE));
        $data['selectdpid'] = $this->session->userdata('selectdpid');
        if (!empty($dpid)){
            $this->session->set_userdata("selectdpid", $dpid);
        } 
		if($data['selectdpid']==""||$data['selectdpid']=="0"||$data['selectdpid']==null) $data['selectdpid']="0";
        //hnw end
