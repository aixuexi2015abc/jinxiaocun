		...
		if (is_array($data)&&count($data)>0) {
			...
			if (count($user)>0) {
				...
				if ($user['userpwd'] == md5($data['userpwd'])) {
					...
					$data['jxcsys']['dianpuid'] = $user['dianpuid'];
                    ...
                    $ids = explode(',',  $user['dianpuid']);//hnw start
                    $data['jxcsys']['selectdpid'] = $ids[0];//hnw start
					if($user['dianpuid']==null||$user['dianpuid']==""){$user['dianpuid']="0";}
                    $dp = $this->db->query(" select * from ci_dianpu where id in (".$user['dianpuid'].")")->result_array();
                    $data['jxcsys']['dianpulist'] = $dp;
					$this->input->set_cookie('ispwd',$data['ispwd'],3600000);
					$this->session->set_userdata($data);
                    $this->session->set_userdata("selectdpid", $ids[0]);//hnw start
