...
<link href="<?php echo base_url()?>statics/css/<?php echo sys_skin()?>/default.css" rel="stylesheet" type="text/css" id="defaultFile">
<script src="<?php echo skin_url()?>/js/common/tabs.js"></script>
<!-- author：209887082 -->
...
<div id="container" class="cf">
  ...
  <div id="col-main">
    <div id="main-hd" class="cf">
      <div class="tit"> 
	  <select id="mySelectaaa" onchange="fnSelectDp()" style="font-size:16px;margin-top:10px;">
                          <?php foreach ($dianpulist as $item):?>
                          <?php if ($selectdpid == $item['id']): ?>
                                 <option selected  value="<?= $item['id'] ?>">
                                     <?= $item['dianpumc'] ?>
                                 </option>
                              <?php else:?>
                                  <option  value="<?= $item['id'] ?>" ) ><?= $item['dianpumc'] ?></option>
                          <?php endif;?>
                          <?php endforeach;?>
                      </select>
		  <a class="company" id="companyName" href="javascript:;" style="display:none;" title=""></a>
		  <span class="period" id="period"></span> </div>
      <ul class="user-menu" style="font-size:14px;padding-top:20px;padding-right:10px;">
      	<li id="sysSkin" style="display:none;">换肤</li>
        <li><a href="<?php echo site_url('login/out')?>" style="font-family:微软雅黑;color:#1b96a9;">退出登录</a></li>
      </ul>  
    </div>
    <div id="main-bd">
...
<script src="<?php echo base_url()?>statics/js/dist/default.js"></script>
<script>
    function fnSelectDp(){
     var options=$("#mySelectaaa option:selected");  //获取选中的项
        var url = "<?php echo site_url('home/index');?>"+"?dpid="+options.val();
